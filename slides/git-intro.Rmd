...
- Default branch names changed from master to main on Github and Gitlab
- Depending on settings: local default branches might be called master instead of main
- You can rename the default branch before pushing with 
...
(- `git branch -M main`: rename branch to main if it's called master by default (optional))

- `git push -u origin main`: push local changes to remote
  - `-u origin main` only when pushing for first time. Afterwards just `git push`
...
- There are many git GUI clients
  ...
  - Most IDEs have a git GUI integrated (e.g. RStudio -> tommorrow)
