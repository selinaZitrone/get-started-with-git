...
- Software that tracks changes in any set of files
...
- Safe (e.g. no accidental loss of versions) `r fontawesome::fa("check-square", fill = "green")`
  - Multiple copies of same repository 
...
- Repository on remote server for everyone to access `r fontawesome::fa("check-square", fill = "green")`
  - Distributed idea: many local repositories synced via one remote repo
...
Git is a huge topic because it is so powerful. Most of the things are best to learn as you need them.
...
- `git log --oneline` compact version of log
- `git log --graph` graphical representation (interesting if you have a branching workflow)
]
